...
Instead, uv supports its own environment variables, like `UV_INDEX_URL`. uv also supports persistent configuration in a `uv.toml` file or a `[tool.uv.pip]` section of `pyproject.toml`. For more information, see [Configuration files](https://jiezhi.github.io/dochub/uv/concepts/configuration-files/index.md).
...
uv also supports pinning packages to dedicated indexes (see: [*Indexes*](https://jiezhi.github.io/dochub/uv/concepts/indexes/#pinning-a-package-to-an-index)), such that a given package is *always* installed from a specific index.
...
For more, see ["Using arbitrary Python environments"](https://jiezhi.github.io/dochub/uv/pip/environments/#using-arbitrary-python-environments).
...
Skipping bytecode compilation can be undesirable in workflows; for example, we recommend enabling bytecode compilation in [Docker builds](https://jiezhi.github.io/dochub/uv/guides/integration/docker/index.md) to improve startup times (at the cost of increased build times).
