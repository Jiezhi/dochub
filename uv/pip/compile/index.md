...
uv allows dependencies to be locked in the `requirements.txt` format. It is recommended to use the standard `pyproject.toml` to define dependencies, but other dependency formats are supported as well. See the documentation on [declaring dependencies](https://jiezhi.github.io/dochub/uv/pip/dependencies/index.md) for more details on how to define dependencies.
...
Dependencies can be installed directly from their definition files or from compiled `requirements.txt` files with `uv pip install`. See the documentation on [installing packages from files](https://jiezhi.github.io/dochub/uv/pip/packages/#installing-packages-from-files) for more details.
