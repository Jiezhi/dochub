...
Alternatively, the [`UV_PROJECT_ENVIRONMENT` setting](https://jiezhi.github.io/dochub/uv/concepts/projects/config/#project-environment-path) can be set before syncing to install to the system Python environment and skip environment activation entirely.

### [Using installed tools](#using-installed-tools)

To use installed tools, ensure the [tool bin directory](https://jiezhi.github.io/dochub/uv/concepts/tools/#tool-executables) is on the path:
...
Changing the [`UV_LINK_MODE`](https://jiezhi.github.io/dochub/uv/guides/reference/settings.md#link-mode) silences warnings about not being able to link files since the cache and sync target are on separate file systems.
