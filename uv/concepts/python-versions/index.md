...
See the [storage documentation](https://jiezhi.github.io/dochub/uv/reference/storage/#python-versions) for details about where installed Python versions are stored.

### [Installing Python executables](#installing-python-executables)

uv installs Python executables into your `PATH` by default, e.g., on Unix `uv python install 3.12` will install a Python executable into `~/.local/bin`, e.g., as `python3.12`. See the [storage documentation](https://jiezhi.github.io/dochub/uv/reference/storage/#python-executables) for more details about the target directory.
...
See the [`uv python list`](https://jiezhi.github.io/dochub/uv/concepts/reference/cli.md#uv-python-list) reference for more details.
...
The [`python-downloads`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#python-downloads) option can be used to disable this behavior. By default, it is set to `automatic`; set to `manual` to only allow Python downloads during `uv python install`.

Tip

The `python-downloads` setting can be set in a [persistent configuration file](https://jiezhi.github.io/dochub/uv/concepts/configuration-files/index.md) to change the default behavior, or the `--no-python-downloads` flag can be passed to any uv command.
...
The [`python-preference`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#python-preference) setting determines whether to prefer using Python installations that are already present on the system, or those that are downloaded and installed by uv.
