...
The `pyproject.toml` includes basic metadata. It does not include a build system, it is not a [package](https://jiezhi.github.io/dochub/uv/concepts/projects/config/#project-packaging) and will not be installed into the environment:
...
Many use-cases require a [package](https://jiezhi.github.io/dochub/uv/concepts/projects/config/#project-packaging). For example, if you are creating a command-line interface that will be published to PyPI or if you want to define tests in a dedicated directory.
...
A [build system](https://jiezhi.github.io/dochub/uv/concepts/projects/config/#build-systems) is defined, so the project will be installed into the environment:
...
A [command](https://jiezhi.github.io/dochub/uv/concepts/projects/config/#entry-points) definition is included:
...
When creating a project with maturin or scikit-build-core, uv configures [`tool.uv.cache-keys`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#cache-keys) to include common source file types. To force a rebuild, e.g. when changing files outside `cache-keys` or when not using `cache-keys`, use `--reinstall`.
