...
When using `uv build`, uv acts as a [build frontend](https://peps.python.org/pep-0517/#terminology-and-goals) and only determines the Python version to use and invokes the build backend. The details of the builds, such as the included files and the distribution filenames, are determined by the build backend, as defined in [`[build-system]`](https://jiezhi.github.io/dochub/uv/concepts/projects/config/#build-systems). Information about build configuration can be found in the respective tool's documentation.
...
Projects can also specify build constraints in [`build-constraint-dependencies`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#build-constraint-dependencies). To verify a downloaded build dependency, provide its `requirement` and `hashes` in a table:
