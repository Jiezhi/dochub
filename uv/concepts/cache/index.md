...
To incorporate additional information into the cache key for a given package, you can add cache key entries under [`tool.uv.cache-keys`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#cache-keys), which covers both file paths and Git commit hashes. Setting [`tool.uv.cache-keys`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#cache-keys) will replace defaults, so any necessary files (like `pyproject.toml`) should still be included in the user-defined cache keys.
...
By default, cache cleanup reports the logical size of removed entries. Enable the `cache-physical-space` [preview feature](https://jiezhi.github.io/dochub/uv/concepts/preview/index.md) to instead report the physical disk space reclaimed, accounting for hardlinks and copy-on-write clones:
...
1. The specific cache directory specified via `--cache-dir`, `UV_CACHE_DIR`, or [`tool.uv.cache-dir`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#cache-dir).
