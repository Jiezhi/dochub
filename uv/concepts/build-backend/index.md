...
The uv build backend currently **only supports pure Python code**. An alternative backend is required to build a [library with extension modules](https://jiezhi.github.io/dochub/uv/concepts/projects/init/#projects-with-extension-modules).
...
To use uv as a build backend in an existing project, add `uv_build` to the [`[build-system]`](https://jiezhi.github.io/dochub/uv/concepts/projects/config/#build-systems) section in your `pyproject.toml`:
...
The uv build backend follows the same [versioning policy](https://jiezhi.github.io/dochub/uv/reference/policies/versioning/index.md) as uv. Including an upper bound on the `uv_build` version ensures that your package continues to build correctly as new versions are released.
...
When the project is built, e.g., with [`uv build`](https://jiezhi.github.io/dochub/uv/guides/package/index.md), the uv build backend will be used to create the source distribution and wheel.
...
- The [module](#modules) under [`tool.uv.build-backend.module-root`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#build-backend_module-root).
- The files referenced by `project.license-files` and `project.readme`.
- All directories under [`tool.uv.build-backend.data`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#build-backend_data).
- All files matching patterns from [`tool.uv.build-backend.source-include`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#build-backend_source-include).

From these, items matching [`tool.uv.build-backend.source-exclude`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#build-backend_source-exclude) and the [default excludes](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#build-backend_default-excludes) are removed.
...
- All directories under [`tool.uv.build-backend.data`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#build-backend_data), which are copied into the `.data` directory.

From these, [`tool.uv.build-backend.source-exclude`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#build-backend_source-exclude), [`tool.uv.build-backend.wheel-exclude`](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#build-backend_wheel-exclude) and the default excludes are removed. The source dist excludes are applied to avoid source tree to wheel builds including more files than source tree to source distribution to wheel build.

There are no specific wheel includes. There must only be one top level module, and all data files must either be under the module root or in the appropriate [data directory](https://jiezhi.github.io/dochub/uv/concepts/reference/settings.md#build-backend_data). Most packages store small data in the module root alongside the source code.
